...
use crate::{lexer::Lexer, measurement::Measurement, value::pow, value::Value};
use std::fmt;
use std::panic;
...
fn expr_bp(lexer: &mut Lexer, min_bp: u8) -> S {
    ...
    let mut lhs = match first_token {
        Token::PosNum(_) | Token::EulersNum | Token::Pi => S::Atom(first_token),
        Token::LeftParen => {
            ...
            match op {
                Token::Add => {
                    if sub_expressions.len() != 2 {
                        panic!(
                            "bad sub-expressions: {:?}, addition ('+') operator is binary.",
                            sub_expressions
                        )
                    } else {
                ...
                Token::Minus => {
                    if sub_expressions.len() == 1 {
                        ...
                        panic!(
                            "bad sub-expressions: {:?}, subtraction ('-') operator is binary.",
                            sub_expressions
                        )
                    } else {
                        let lhs = eval_expr(&sub_expressions[0]);
                ...
                Token::Mul => {
                    if sub_expressions.len() != 2 {
                        panic!(
                            "bad sub-expressions: {:?}, multiplication ('*') operator is binary.",
                            sub_expressions
                        )
                    } else {
                        let lhs = eval_expr(&sub_expressions[0]);
                ...
                Token::Div => {
                    if sub_expressions.len() != 2 {
                        panic!(
                            "bad sub-expressions: {:?}, division ('/') operator is binary.",
                            sub_expressions
                        )
                    } else {
                        let lhs = eval_expr(&sub_expressions[0]);
                ...
                Token::Caret => {
                    if sub_expressions.len() != 2 {
                        panic!(
                            "bad sub-expressions: {:?}, exponentiation ('^') operator is binary.",
                            sub_expressions
                        )
                    } else {
                        let lhs = eval_expr(&sub_expressions[0]);
                ...
                Token::PlusMinus => {
                    if sub_expressions.len() != 2 {
                        panic!(
                            "bad sub-expressions: {:?}, plus-minus ('±') operator is binary.",
                            sub_expressions
                        )
                    } else {
                        let lhs = eval_expr(&sub_expressions[0]);
                        ...
                        let x = match lhs {
                            ...
                            _ => panic!(
                                "left-hand side is not a number! lhs: {:?}",
                                sub_expressions[0]
                            ),
                        };
                        let y = match rhs {
                            Value::PosNumber(m) => m,
                            _ => panic!(
                                "right-hand side is not a positive number! rhs: {:?}",
                                sub_expressions[1]
                            ),
                        };
